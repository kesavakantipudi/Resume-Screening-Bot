...
async def test_primary_model_succeeds_no_fallback():
    provider = GeminiProvider(api_key="mock_key", model_name="gemini-3.5-flash-lite")
    attempted_models = []
    ...
    assert provider.last_used_model == "gemini-3.5-flash-lite"
    assert attempted_models == ["gemini-3.5-flash-lite"]
...
async def test_primary_hits_429_fallback_to_secondary():
    ...
    async def mock_call(model_name: str):
        ...
        if model_name == "gemini-3.5-flash-lite":
            raise Exception("429 ResourceExhausted: Quota exceeded for gemini-3.5-flash-lite")
    ...
    assert provider.last_used_model == "gemini-3.1-flash-lite"
...
async def test_primary_and_fallback1_hit_quota_fallback_to_tertiary():
    ...
    assert attempted_models.count("gemini-3.5-flash-lite") == settings.GEMINI_MAX_RETRIES + 1
    assert attempted_models.count("gemini-3.1-flash-lite") == settings.GEMINI_MAX_RETRIES + 1
    assert "gemini-3.6-flash" in attempted_models
