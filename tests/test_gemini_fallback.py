...
async def test_primary_model_succeeds_no_fallback():
    provider = GeminiProvider(api_key="mock_key", model_name="gemini-3.8-flash")
    attempted_models = []
...
async def test_primary_hits_429_fallback_to_secondary():
    ...
    async def mock_call(model_name: str):
        ...
        if model_name == "gemini-3.8-flash":
            raise Exception("429 ResourceExhausted: Quota exceeded for gemini-3.8-flash")
    ...
    assert provider.last_used_model == "gemini-3.5-flash"
    assert "gemini-3.8-flash" in attempted_models
...
async def test_primary_and_fallback1_hit_quota_fallback_to_tertiary():
    ...
    async def mock_call(model_name: str):
        attempted_models.append(model_name)
        if model_name in ["gemini-3.8-flash", "gemini-3.5-flash"]:
    ...
    assert attempted_models.count("gemini-3.8-flash") == settings.GEMINI_MAX_RETRIES + 1
    assert attempted_models.count("gemini-3.5-flash") == settings.GEMINI_MAX_RETRIES + 1
